...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  if(type(modeling.heads) != type(None)):
    ...
    for i in range(modeling.heads):
      mh0_i = tf.get_default_graph().get_tensor_by_name('pooler/mh0_'+str(i))
      mh1_i = tf.get_default_graph().get_tensor_by_name('pooler/mh1_'+str(i))
      for j in range(i+1, modeling.heads):
        mh0_j = tf.get_default_graph().get_tensor_by_name('pooler/mh0_'+str(j))
        mh1_j = tf.get_default_graph().get_tensor_by_name('pooler/mh1_'+str(j))
